    ...
    .then((H) => {
      ...
      const buildItemsKey = (items) =>
        (items || [])
          .map(
            (it) =>
              `${toInt(it.item_id)}:${toInt(it.qty)}:${it.title || ''}:${it.image_url || ''}`,
          )
          .join('|');
      ...
      const renderChest = (H, cfg, ui, inv, onOpen, state) => {
        ...
        if (purchaseEnabled) {
          const purchaseBox = H.createEl('div', { className: 'ks-drops-chest__buy' });
          purchaseBox.appendChild(
            H.createEl('div', {
              className: 'ks-drops-chest__currency',
              text: `${purchaseCfg.currencyLabel || 'Валюта'}: ${rawCurrency ?? '0'}`,
            }),
          );
          purchaseBox.appendChild(
            H.createEl('div', {
          ...
          const buyRow = H.createEl('div', { className: 'ks-drops-chest__buyrow' });
      ...
      const renderBank = (H, ui, bank, state) => {
        ...
        head.appendChild(
          H.createEl('div', { className: 'ks-drops-bank__title', text: 'Склад' }),
        );
        head.appendChild(
          H.createEl('div', {
        ...
        if (!items.length) {
          grid.appendChild(
            H.createEl('div', { className: 'ks-drops-bank__empty', text: 'Пока пусто.' }),
          );
        ...
        for (const it of items) {
          ...
          const imgWrap = H.createEl('div', { className: 'ks-drops-bank__imgwrap' });
          imgWrap.appendChild(H.createEl('img', { src: it.image_url || '', alt: it.title || '' }));
          ...
          meta.appendChild(
            H.createEl('div', { className: 'ks-drops-bank__qty', text: 'x' + (it.qty ?? 0) }),
          );
      ...
      const renderBuildings = (H, cfg, ui, buildingsState, bank, state) => {
        ...
        wrap.appendChild(
          H.createEl('div', { className: 'ks-drops-buildings__title', text: 'Здания и помещения' }),
        );

        const grid = H.createEl('div', { className: 'ks-drops-buildings__grid' });
        ...
        for (const b of buildings) {
          ...
          const imgWrap = H.createEl('div', { className: 'ks-drops-buildings__imgwrap' });
          imgWrap.appendChild(
            H.createEl('img', {
              src: b.image_url || '',
              alt: b.title || '',
          ...
          const meta = H.createEl('div', { className: 'ks-drops-buildings__meta' });
          meta.appendChild(
            H.createEl('div', {
          ...
          if (b.description) {
            meta.appendChild(
              H.createEl('div', { className: 'ks-drops-buildings__desc', text: b.description }),
            );
          }
          ...
          if (!resources.length) {
            ...
            for (const r of resources) {
              ...
              const res = H.createEl('div', {
                className: ['ks-drops-buildings__res', enough ? 'is-enough' : '']
                  .filter(Boolean)
                  .join(' '),
              });
              ...
              res.appendChild(
                H.createEl('div', { className: 'ks-drops-buildings__resqty', text: `${have}/${need}` }),
              );
      ...
      const renderVoting = (H, cfg, ui, buildingsState, state, handlers) => {
        ...
        const wrap = H.createEl('div', { className: 'ks-drops-vote__wrap' });
        wrap.appendChild(
          H.createEl('div', {
        ...
        if (!available.length) {
          list.appendChild(
            H.createEl('div', { className: 'ks-drops-vote__empty', text: 'Нет доступных зданий для голосования.' }),
          );
        } else {
          for (const b of available) {
            ...
            const left = H.createEl('div', { className: 'ks-drops-vote__left' });
            if (b.image_url) {
              left.appendChild(
                H.createEl('img', {
            ...
            left.appendChild(
              H.createEl('div', { className: 'ks-drops-vote__name', text: b.title || b.id }),
            );
            row.appendChild(left);

            row.appendChild(
              H.createEl('div', { className: 'ks-drops-vote__count', text: `Голоса: ${counts[b.id] || 0}` }),
            );
      ...
      const renderInventory = (H, cfg, ui, inv, bank, buildingsState, handlers, state) => {
        ...
        if (!items.length) {
          frag.appendChild(
            H.createEl('div', { className: 'ks-drops-inv__empty', text: 'Пока пусто.' }),
          );
        ...
        for (const it of items) {
          ...
          const imgWrap = H.createEl('div', { className: 'ks-drops-inv__imgwrap' });
          imgWrap.appendChild(H.createEl('img', { src: it.image_url || '', alt: it.title || '' }));
          ...
          meta.appendChild(
            H.createEl('div', { className: 'ks-drops-inv__qty', text: 'x' + (it.qty ?? 0) }),
          );
      ...
      const init = async () => {
        ...
        const ui = {
          ...
          bankBox: H.createEl('div', { className: 'ks-drops-bank ks-drops-bankbox' }),
        ...
        const rerender = () => {
          ...
          renderInventory(H, cfg, ui, inv, bank, buildingsState, handlers, renderState);
        };
        ...
        const handlers = {
          onOpenChest: async (btn, purchaseQty = null, currencyValue = null) => {
            if (purchaseQty) {
              ...
              try {
                const resp = await H.request(apiUrl('purchase_request', {}), {
                  method: 'POST',
                  timeout: cfg.polling?.requestTimeoutMs || 12000,
                  responseType: 'json',
                  headers: { 'Content-Type': 'application/json' },
                ...
                const d = resp.data || {};
                if (!d.success) toast(d.message || 'Ошибка заявки', 'error');
                else toast(d.message || 'Заявка отправлена', 'success');
              } catch {
            ...
            try {
              ...
              if (!d.opened) {
                ...
                if (reward.type === 'none') {
                  toast(cfg.chest?.texts?.nothing || 'Ничего не выпало', 'info');
                } else {
              ...
              try { bank = await fetchBankState(); } catch {}
              scheduleRender();
            } catch (e) {
              log('chest open error', e);
              toast(cfg.chest?.texts?.error || 'Не удалось открыть сундук', 'error');
            } finally {
              btn.classList.remove('is-busy');
          ...
          onVote: async (buildingId, btn) => {
            ...
            try {
              ...
              if (!res.success) toast(res.message || 'Ошибка голосования', 'error');
              else toast(res.message || 'Голос учтен', 'success');

              if (res.buildings_state) buildingsState = res.buildings_state;
              scheduleRender();
            } catch (e) {
              log('vote error', e);
        ...
        try {
          ...
          buildingsState = buildingsRes.status === 'fulfilled' ? buildingsRes.value : null;

          scheduleRender();
        } catch (e) {
          log('init load error', e);
          toast('Не удалось загрузить инвентарь/банк', 'error');
          root.innerHTML = '<div class="ks-drops-inv__empty">Ошибка загрузки.</div>';
          return;
        }
