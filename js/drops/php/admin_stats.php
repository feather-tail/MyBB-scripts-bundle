...
function building_label(string $buildingId, array $buildingMap): string {
  $title = trim((string)($buildingMap[$buildingId]['title'] ?? ''));
  if ($title !== '') {
...
$tVotes = drops_buildings_votes_table($cfg);
if ($tVotes === '' || !drops_db_table_exists($pdo, $tVotes)) {
  $warnings[] = 'Таблица голосования за строительство недоступна (building_votes не найдена).';
} else {
  $sqlV = "SELECT user_id, building_id, created_at, updated_at FROM `$tVotes` ORDER BY updated_at DESC";
  ...
  while ($row = $stmtV->fetch(PDO::FETCH_ASSOC)) {
    ...
    $buildingId = trim((string)($row['building_id'] ?? ''));
    ...
    $votesByBuilding[$buildingId] = $votesByBuilding[$buildingId] ?? [];
    $votesByBuilding[$buildingId][] = $uid;
...
<h2>5) Голосование за следующее строительство</h2>
<div class="box">
  <div class="row muted" style="margin-bottom:8px">
    <div>Голосов всего: <span class="pill"><?= fmt_int(count($votesByUser)) ?></span></div>
    <div>Строк в таблице голосования: <span class="pill"><?= fmt_int($voteRows) ?></span></div>
  </div>

  <details open>
    <summary><b>Кто как голосовал (по пользователям)</b></summary>
    <div style="margin-top:10px">
      <table>
        <thead>
          <tr>
        ...
        <tbody>
        <?php
          $voteList = array_values($votesByUser);
          usort($voteList, fn($a, $b) => strcmp((string)($b['updated_at'] ?? ''), (string)($a['updated_at'] ?? '')));
          foreach ($voteList as $vote):
            $bid = (string)($vote['building_id'] ?? '');
        ?>
          <tr>
            <td><?= user_link((int)($vote['user_id'] ?? 0), $cfg, $userMap) ?></td>
            <td><?= h(building_label($bid, $buildingMap)) ?></td>
            <td class="right"><span class="muted"><?= h((string)($vote['updated_at'] ?? '')) ?></span></td>
          </tr>
        <?php endforeach; ?>
        <?php if (!$voteList): ?>
          <tr><td colspan="3" class="muted">Пока нет голосов.</td></tr>
        <?php endif; ?>
        </tbody>
      </table>
    </div>
  ...
  <details style="margin-top:10px">
    <summary><b>Голоса по зданиям</b></summary>
    <div style="margin-top:10px">
      <table>
        <thead>
          <tr>
            <th>Здание</th>
            <th class="right">Голосов</th>
            <th>Кто проголосовал</th>
          </tr>
        </thead>
        <tbody>
        <?php
          $voteBuildings = array_keys($votesByBuilding);
          usort($voteBuildings, function ($a, $b) use ($votesByBuilding, $buildingMap) {
            $cntA = count($votesByBuilding[$a] ?? []);
            $cntB = count($votesByBuilding[$b] ?? []);
            if ($cntA !== $cntB) return $cntB <=> $cntA;
            $titleA = (string)($buildingMap[$a]['title'] ?? $a);
            $titleB = (string)($buildingMap[$b]['title'] ?? $b);
            return strcmp($titleA, $titleB);
          });

          foreach ($voteBuildings as $bid):
            $uids = $votesByBuilding[$bid] ?? [];
            sort($uids);
            $links = array_map(fn($uid) => user_link((int)$uid, $cfg, $userMap), $uids);
        ?>
          <tr>
            <td><?= h(building_label($bid, $buildingMap)) ?></td>
            <td class="right"><b><?= fmt_int(count($uids)) ?></b></td>
            <td><?= $links ? implode(', ', $links) : '<span class="muted">—</span>' ?></td>
          </tr>
        <?php endforeach; ?>
        <?php if (!$voteBuildings): ?>
          <tr><td colspan="3" class="muted">Пока нет голосов.</td></tr>
        <?php endif; ?>
        </tbody>
      </table>
