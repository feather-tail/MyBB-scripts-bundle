(() => {
  ...
  const cmModalLock = (() => {
    ...
    const on = () => {
      n += 1;
      if (n === 1) document.body.classList.add("cm-modal-open");
    };
    const off = () => {
      n = Math.max(0, n - 1);
      if (n === 0) document.body.classList.remove("cm-modal-open");
    };
    const forceOff = () => {
      n = 0;
      document.body.classList.remove("cm-modal-open");
    };
    return { on, off, forceOff };
  })();
  ...
  const enhanceCharacter = (character, { uid, close, backdropEl } = {}) => {
    ...
    // UPDATED: apply stage class to backdrop (prefer passed element)
    const backdrop = backdropEl || findBackdropCandidate(character);
    if (backdrop) backdrop.classList.add("cm-backdrop");
    ...
    // UPDATED: правильное добавление/удаление обработчика
  ...
  function init() {
    document.body.addEventListener("click", async (e) => {
      ...
      // NEW: lock scroll immediately
      cmModalLock.on();
  
      // NEW: find backdrop immediately (stage)
      const stageBackdrop = findBackdropCandidate(box);
      if (stageBackdrop) stageBackdrop.classList.add("cm-backdrop");
      ...
      const closeAll = () => {
        ...
        // cleanup stage + lock
        if (stageBackdrop) stageBackdrop.classList.remove("cm-backdrop");
        cmModalLock.off();
      ...
      try {
        ...
        // UPDATED: pass backdrop element
        enhanced = enhanceCharacter(root, { uid: targetUid, close: closeOrig, backdropEl: stageBackdrop });
